...
export default function PercentileInteractive() {
  ...
  return (
    <div dir="rtl" style={{ ...graphCardStyle }}>
      <h3 style={graphTitleStyle}>מהו אחוזון, וכמה מהערכים נמצאים מתחתיו?</h3>
      <p style={graphSubtitleStyle}>
        50 ציונים מסודרים בהיסטוגרמת נקודות מהנמוך לגבוה. הזיזו את הסליידר לאחוזון שתרצו (למשל P25 — 25% מהערכים מתחתיו): הקו הצהוב מסמן את הערך באותו אחוזון, והתיבה למטה סופרת בזמן אמת כמה נקודות נמצאות מתחתיו ומעליו.
      </p>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
        ...
        {positions.map(pos => (
          <circle
            ...
            cy={pos.cy}
            r={dotR}
            fill={pos.below ? GC.blue : GC.axisText}
            stroke={GC.ink}
            strokeWidth={1}
            opacity={0.95}
          />
        ...
        <line
          ...
          strokeDasharray="6 4"
        />
        <text x={toX(value)} y={PAD_Y - 6} fill={GC.goldText} fontSize={13} textAnchor="middle" fontWeight={700}>
          P{p} = {value.toFixed(2)}
        </text>
      </svg>
      ...
      <div
        style={{
          marginTop: 10,
          padding: '10px 14px',
          background: 'rgba(78,113,218,0.10)',
          borderRadius: 8,
          fontSize: 13,
          display: 'flex',
          gap: 16,
      ...
      >
        <span><span style={{ color: GC.blue, fontWeight: 700 }}>{belowCount}</span> מתחת ל-P{p}</span>
        <span><span style={{ color: GC.axisText, fontWeight: 700 }}>{N - belowCount}</span> מעליו</span>
        <span style={{ marginInlineStart: 'auto' }}>ערך באחוזון: <span style={{ color: GC.goldText, fontWeight: 700 }}>{value.toFixed(2)}</span></span>
      </div>
