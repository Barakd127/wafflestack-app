/**
 * StdDevTwoDistributions — two overlapping bell curves with same μ but
 * different σ. Slider for σ₂. Shows "same centre, different spread" visually.
 */
import { useState, useEffect } from 'react'
import { GRAPH_FONT, GC, graphCardStyle, graphTitleStyle, graphSubtitleStyle } from './graphTheme'

const W = 640, H = 320, PAD = 50
const X0 = PAD, X1 = W - PAD, Y0 = PAD, Y1 = H - 70

const phi = (z: number) => Math.exp(-0.5 * z * z) / Math.sqrt(2 * Math.PI)

export default function StdDevTwoDistributions() {
  const [sigma1, setSigma1] = useState(1)
  const [sigma2, setSigma2] = useState(2.5)
  const mu = 0
  const xMin = -8, xMax = 8

  const toPx = (v: number) => X0 + ((v - xMin) / (xMax - xMin)) * (X1 - X0)
  const yMax = 0.42 / Math.min(sigma1, sigma2)
  const toY = (v: number) => Y1 - (v / yMax) * (Y1 - Y0)

  const curve = (sigma: number) => {
    const pts: string[] = []
    for (let p = X0; p <= X1; p += 2) {
      const x = xMin + ((p - X0) / (X1 - X0)) * (xMax - xMin)
      const y = phi((x - mu) / sigma) / sigma
      pts.push(`${p},${toY(y)}`)
    }
    return `M ${pts.join(' L ')}`
  }

  useEffect(() => {
    const k = (window as { katex?: { render: (s: string, el: HTMLElement, o?: object) => void } }).katex
    const el = document.getElementById('two-dist-formula')
    if (k && el) k.render(`\\sigma_1 = ${sigma1.toFixed(1)}, \\ \\sigma_2 = ${sigma2.toFixed(1)}, \\ \\text{same } \\mu = 0`, el, { throwOnError: false })
  }, [sigma1, sigma2])

  return (
    <div dir="rtl" style={graphCardStyle}>
      <h3 style={graphTitleStyle}>שתי התפלגויות — אותו מרכז, פיזור שונה</h3>
      <p style={graphSubtitleStyle}>גרור את הסטיות. ככל ש-σ גדלה, העקומה רחבה ושטוחה יותר. אותו μ — אך פיזור שונה לחלוטין.</p>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
        <path d={curve(sigma1)} stroke={GC.ink} strokeWidth={2.5} fill="rgba(31,62,108,0.18)" />
        <path d={curve(sigma2)} stroke={GC.gold} strokeWidth={2.5} fill={GC.goldFill} />
        <line x1={X0} y1={Y1} x2={X1} y2={Y1} stroke={GC.axis} />
        <line x1={toPx(mu)} y1={Y0 - 6} x2={toPx(mu)} y2={Y1} stroke={GC.axis} strokeDasharray="4 4" />
        <text x={toPx(mu)} y={Y0 - 10} fill={GC.axisText} fontSize={12} fontFamily={GRAPH_FONT} textAnchor="middle">μ = 0</text>
        {[-6, -4, -2, 0, 2, 4, 6].map(t => (
          <text key={t} x={toPx(t)} y={Y1 + 18} fill={GC.axisText} fontSize={11} fontFamily={GRAPH_FONT} textAnchor="middle">{t}</text>
        ))}
        <text x={X1 - 6} y={Y0 + 14} fill={GC.ink} fontSize={12} fontFamily={GRAPH_FONT} textAnchor="end">σ₁ = {sigma1.toFixed(1)}</text>
        <text x={X1 - 6} y={Y0 + 30} fill={GC.gold} fontSize={12} fontFamily={GRAPH_FONT} textAnchor="end">σ₂ = {sigma2.toFixed(1)}</text>
      </svg>
      <div id="two-dist-formula" style={{ textAlign: 'center', margin: '8px 0', minHeight: 28 }} />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 8, fontFamily: GRAPH_FONT }}>
        <label style={{ fontSize: 12 }}>σ₁: {sigma1.toFixed(1)}<input type="range" min={0.3} max={4} step={0.1} value={sigma1} onChange={e => setSigma1(+e.target.value)} style={{ width: '100%', accentColor: GC.blue }} /></label>
        <label style={{ fontSize: 12 }}>σ₂: {sigma2.toFixed(1)}<input type="range" min={0.3} max={4} step={0.1} value={sigma2} onChange={e => setSigma2(+e.target.value)} style={{ width: '100%', accentColor: GC.blue }} /></label>
      </div>
    </div>
  )
}
