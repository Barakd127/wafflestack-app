      ...
      {!isDone && isMobile && tab === 'canvas' && (
        <div data-tour="canvas-frame" style={{ position: 'fixed', inset: 0, zIndex: 400, background: '#0d1628' }}>
          <iframe
            ...
            src={`${import.meta.env.BASE_URL}mindmap.html?v=mm6-20260627&mode=wb&userId=${userId || 'default'}&wbScene=q-${encodeURIComponent(q?.id ?? `idx${currentQ}`)}`}
            title="קנבס לשאלה"
        ...
        {!isDone && tab !== 'none' && (
          <div style={
            ...
            {tab === 'mindmap' && (
              <iframe
                key="quiz-mm"
                src={`${import.meta.env.BASE_URL}mindmap.html?v=mm6-20260627&userId=${userId || 'default'}${selectedTopic ? `&topic=${encodeURIComponent(selectedTopic)}` : ''}`}
