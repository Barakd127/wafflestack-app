  ...
  return (
    <div className="ws-screen-pad" style={viewMode === 'mindmap' ? { flex: 1, minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '5px 10px 8px' } : { flex: 1, overflow: 'auto', padding: '32px 40px' }}>
      ...
      {viewMode === 'list' ? (
        ...
        <iframe
          src={`${import.meta.env.BASE_URL}mindmap.html?v=mm13-20260630&scene=topics&admin=${_adminMode ? '1' : '0'}`}
          title="מפת הנושאים"
      ...
      {!isDone && isMobile && tab === 'canvas' && (
        <div data-tour="canvas-frame" style={{ position: 'fixed', inset: 0, zIndex: 400, background: '#0d1628' }}>
          <iframe
            ...
            src={`${import.meta.env.BASE_URL}mindmap.html?v=mm13-20260630&mode=wb&userId=${userId || 'default'}&wbScene=q-${encodeURIComponent(q?.id ?? `idx${currentQ}`)}`}
            title="קנבס לשאלה"
        ...
        {!isDone && tab !== 'none' && (
          <div style={
            ...
            {tab === 'mindmap' && (
              <iframe
                key="quiz-mm"
                src={`${import.meta.env.BASE_URL}mindmap.html?v=mm13-20260630&userId=${userId || 'default'}${selectedTopic ? `&topic=${encodeURIComponent(selectedTopic)}` : ''}`}
